...
def main(minutes: float = 10.0) -> None:
    ...
    pipeline = Pipeline(source, on_subtitle, model="medium", language="es")  # pin Spanish (D-017)
    print("loading model (medium/int8, source=es) and starting capture... play a video now.")
    pipeline.start()
    try:
